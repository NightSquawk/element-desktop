...
Detailed working agreements live in `.claude/rules/` (mirrored under `.codex/rules/`). Read the relevant rule before acting; subagents do **not** inherit these, so paste the relevant rule text into their prompts (see Core Instructions).

| # | Rule | File |
|---|---|---|
| 1 | Core Instructions (behavior, fork scope, subagent briefing) | `.claude/rules/core-instructions.mdc` |
| 2 | Context7 Auto-Use (current library docs) | `.claude/rules/context7.mdc` |
| 3 | Git Workflow (fork/upstream, branches, commits) | `.claude/rules/git-workflow.mdc` |
| 4 | Pre-Stash Build Gate | `.claude/rules/stash-build-gate.mdc` |
